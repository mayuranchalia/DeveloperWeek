...
{	
    	...
    	* 
    	**/
	   remote function getbooks() produces="application/json,application/xml" 
	   httpmethod="GET" description="Get list of books" returntype="Query" {
	   	
	   	resultset = queryExecute("select b.bookid, b.title,b.authorid, b.bookimage,
	   	 b.bookdescription, b.isspotlight, b.genre from books b",[],
	   		{datasource = "cfbookclub"});
	   		if(!resultset.recordcount> 0){
				throw("Books not found", "404", "No books found","404");
			}
			//serializeJSON(resultset)
		return resultset;
	}  
	...
	*/
	remote function getbookDetails(required String bookid restargsource = "path" 
	restargname="bookid" ) restpath="{bookid}" produces="application/json,application/xml" 
	httpmethod="GET" returntype="book" description="Get a book with specific id" 
	responseMessages="404:Not found,200:Successful" {
		...
		resultset = queryExecute("select b.bookid, a.firstname, a.lastname, a.authorid, 
		b.title, b.bookimage, b.bookdescription, b.isspotlight, b.genre
					from books b, authors a
					where a.authorid = b.authorid
					and b.bookid = :bookid", qparams,{datasource = "cfbookclub"});
		if(!resultset.recordcount> 0){
			throw("Book not found", "404", "Book with ID #arguments.bookid# 
			was not found","404");
		}
	...
	*
	*/
	remote function addBook(required Struct structData restargsource = "body" ) 
	consumes="application/json" produces="application/json"  
	    ...
	    try{
	       resultset = queryExecute("insert into books (bookid, authorid, title, 
	       bookimage,thumbnailimage, bookdescription, isspotlight, genre) values
	       	(#structData.bookid#, #structData.authorid#, '#trim(structData.title)#', 
